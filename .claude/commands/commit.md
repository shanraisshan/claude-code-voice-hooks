...
3. **Read `workflow-changelog/workflow-changelog.md`** to get the latest entry's priority actions table. Use the items in that table as the list of changes. If the command name is not `workflows:workflow-changelog`, fall back to reviewing the conversation to identify actions.
